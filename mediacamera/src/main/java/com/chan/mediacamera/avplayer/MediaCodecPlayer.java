package com.chan.mediacamera.avplayer;
...
public class MediaCodecPlayer implements MediaTimeProvider {
     ...
     */
    public MediaCodecPlayer(Surface holder) {
    ...
    private boolean addTrack(int trackIndex, MediaFormat format) throws IOException {
        ...
        codec.configure(
                format,
                isVideo ? mSurfaceHolder : null, null, 0);

        CodecState state;
        if (isVideo) {
            state = new CodecState(this, mVideoExtractor,
                            trackIndex, format, codec, true);
            mVideoCodecStates.put(Integer.valueOf(trackIndex), state);
        } else {
            state = new CodecState(this, mAudioExtractor,
                            trackIndex, format, codec, true);
    ...
    public int getmMediaFormatHeight() {
        return mMediaFormatHeight;
    }

    public int getmMediaFormatWidth() {
